...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['recruiter-team-member', 'supervisor'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<RecruiterInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/recruiters` : null),
    () => getRecruiters({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Recruiter',
      path: isTenantUser ? `/recruiters/view/${data?.[0]?.id}` : '/recruiters',
      entity: 'recruiter',
    ...
    { name: 'Pipeline', path: '/pipelines', entity: 'pipeline', service: AccessServiceEnum.PROJECT, icon: FiLayers },
    {
