...
import { createPipelineCandidate } from 'apiSdk/pipeline-candidates';
...
function PipelineCandidateCreatePage() {
  ...
  const handleSubmit = async (values: PipelineCandidateInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createPipelineCandidate(values);
      resetForm();
  ...
  const formik = useFormik<PipelineCandidateInterface>({
    initialValues: {
      pipeline_id: (router.query.pipeline_id as string) ?? null,
